#!/usr/bin/env python3
"""Safety check script for ROGUE-X trading system."""

import sys
from pathlib import Path

# Allow running directly from a source checkout without installation
sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent))

from rogue_x.config import TradingConfig  # noqa: E402
from rogue_x.core import DataValidator, RiskManager  # noqa: E402


def run_safety_checks() -> bool:
    """Run all safety checks.

    Returns:
        True if all checks pass
    """
    print("=" * 60)
    print("ROGUE-X Trading System Safety Check")
    print("=" * 60)

    all_passed = True

    # Test 1: Configuration validation
    print("\n[1] Testing configuration validation...")
    try:
        config = TradingConfig()
        config.validate()
        print("\u2713 Configuration validation passed")
    except Exception as e:
        print(f"\u2717 Configuration validation failed: {e}")
        all_passed = False

    # Test 2: Risk manager initialization
    print("\n[2] Testing risk manager initialization...")
    try:
        config = TradingConfig()
        risk_mgr = RiskManager(config, 100000.0)
        print("\u2713 Risk manager initialized successfully")
    except Exception as e:
        print(f"\u2717 Risk manager initialization failed: {e}")
        all_passed = False

    # Test 3: Position limits
    print("\n[3] Testing position size limits...")
    try:
        config = TradingConfig(max_position_size=0.02, warmup_bars=0)
        risk_mgr = RiskManager(config, 100000.0)

        # Should pass
        can_open, reason = risk_mgr.can_open_position("AAPL", 15, 100.0, 95.0)  # $1,500 < 2% limit
        assert can_open, f"Valid position rejected: {reason}"

        # Should fail - too large
        can_open, reason = risk_mgr.can_open_position("TSLA", 1000, 300.0, 290.0)
        assert not can_open, "Oversized position was approved"

        print("\u2713 Position size limits working correctly")
    except Exception as e:
        print(f"\u2717 Position size limit test failed: {e}")
        all_passed = False

    # Test 4: Data validation
    print("\n[4] Testing data validation...")
    try:
        # Valid price
        DataValidator.validate_price(100.50, "TEST")

        # Invalid prices should fail
        try:
            DataValidator.validate_price(-10, "TEST")
            raise AssertionError("Negative price was accepted")
        except ValueError:
            pass  # Expected

        print("\u2713 Data validation working correctly")
    except Exception as e:
        print(f"\u2717 Data validation test failed: {e}")
        all_passed = False

    # Test 5: Stop loss requirement
    print("\n[5] Testing stop loss requirements...")
    try:
        config = TradingConfig(require_stop_loss=True, warmup_bars=0)
        risk_mgr = RiskManager(config, 100000.0)

        # Should fail without stop loss
        can_open, reason = risk_mgr.can_open_position("MSFT", 5, 300.0, None)  # $1,500 < 2% limit
        assert not can_open, "Position without stop loss was approved"
        assert "stop loss" in reason.lower()

        print("\u2713 Stop loss requirement enforced")
    except Exception as e:
        print(f"\u2717 Stop loss requirement test failed: {e}")
        all_passed = False

    # Test 6: Warmup gate
    print("\n[6] Testing warmup gate...")
    try:
        config = TradingConfig(warmup_bars=210)
        risk_mgr = RiskManager(config, 100000.0)

        can_open, reason = risk_mgr.can_open_position("AAPL", 5, 100.0, 95.0)
        assert not can_open and "Warmup" in reason, "Position allowed before warmup"

        for _ in range(210):
            risk_mgr.record_bar()
        can_open, reason = risk_mgr.can_open_position("AAPL", 5, 100.0, 95.0)
        assert can_open, f"Position rejected after warmup: {reason}"

        print("\u2713 Warmup gate enforced (210 bars)")
    except Exception as e:
        print(f"\u2717 Warmup gate test failed: {e}")
        all_passed = False

    # Test 7: Circuit breaker
    print("\n[7] Testing circuit breaker...")
    try:
        config = TradingConfig(warmup_bars=0, max_drawdown=0.25)
        risk_mgr = RiskManager(config, 100000.0)

        risk_mgr.current_capital = 70000.0  # simulate 30% realized drawdown
        risk_mgr.check_circuit_breaker()
        assert risk_mgr.trading_halted, "Circuit breaker did not trip at 30% drawdown"

        can_open, reason = risk_mgr.can_open_position("AAPL", 1, 100.0, 95.0)
        assert not can_open and "halted" in reason.lower(), "Halted system approved a position"

        print("\u2713 Circuit breaker halts trading at max drawdown")
    except Exception as e:
        print(f"\u2717 Circuit breaker test failed: {e}")
        all_passed = False

    # Summary
    print("\n" + "=" * 60)
    if all_passed:
        print("\u2713 ALL SAFETY CHECKS PASSED")
        print("=" * 60)
        return True
    else:
        print("\u2717 SOME SAFETY CHECKS FAILED")
        print("=" * 60)
        return False


if __name__ == "__main__":
    success = run_safety_checks()
    sys.exit(0 if success else 1)
