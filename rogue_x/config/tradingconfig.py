...
class TradingConfig:
    ...
    max_drawdown: float = 0.25  # Circuit breaker: halt all trading at 25% drawdown from peak
    ...
    warmup_bars: int = 210  # Bars of data required before any position may be opened
    allow_short: bool = False  # Long-only by default; shorts require margin and explicit opt-in
    ...
    def validate(self) -> bool:
        ...
        if self.max_drawdown <= 0 or self.max_drawdown > 0.50:
            raise ValueError("max_drawdown must be between 0 and 0.50")
